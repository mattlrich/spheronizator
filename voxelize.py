...
def extract_boxes(
    ...
    #output_atoms_invert = 1 - central_res
    #output_atoms_masked = output_atoms[:, :, :, :, :, 0] * output_atoms_invert
    output_atoms_masked = output_atoms


    np.save(os.path.join(atom_out_path, out_name), output_atoms_masked)
    np.save(os.path.join(bond_out_path, out_name), output_bonds)

    print(f"{len(x.output)} voxel boxes/spheres processed, {n_res} residues total.")

    with open(os.path.join(meta_out_path, out_name_meta), 'w') as output:
